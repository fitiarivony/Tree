   ...
   @JsonIgnore
    public Field[] getTraitable() throws MappingException, Exception{
      ...
      public Field[] allAttributes() throws MappingException, Exception{
       ...
       public Field[] allAttributesTrait() throws MappingException, Exception{
        Class cl=this.getClass();
        Field[]fields=cl.getDeclaredFields();
        ArrayList<Field>inserer=new ArrayList<>();
        for(int i=0;i<fields.length;i++){
            if(OutilsBdd.getAttribut(fields[i])!=null && this.get(fields[i])!=null){
                inserer.add(fields[i]);
            }
        }
        return inserer.toArray(new Field[inserer.size()]);
     ...
     public  ArrayList<Object> valeurs(Field[] inserer) throws Exception{
        ...
        for(int i=0;i<inserer.length;i++){
            ...
            }else if(get(this,inserer[i].getName()) instanceof Duration){
                PGInterval interval = new PGInterval(((Duration)(get(this,inserer[i].getName()))).toString());
                valeur.add(interval);
            }
             else{
    ...
    public static Object get(Object o,String attribut) throws Exception{
      ...
      public  Object get(Field attribut) throws Exception{
        ...
        try {
            Method meth=cl.getMethod("get"+fonction,null);
            obj=meth.invoke(this,null);
          return obj;
        } catch (NoSuchMethodException ex) {
          Method[]methods=cl.getMethods();
                for(int j=0;j<methods.length;j++){
                  // System.out.println(methods[j].getName().toLowerCase()+"-----"+"get"+attribut.getName().toLowerCase());
                    if(methods[j].getName().toLowerCase().equalsIgnoreCase("get"+attribut.getName().toLowerCase())){
                        
                        obj=methods[j].invoke(this,null);
                        return obj;
    ...
    //  System.out.println(fonction+"----"+"  "+valeur+" "+OutilsBdd.getField(o, attribut).getType());        
     ...
     public Filtre[] storeRestrictions() throws Exception{
           Field[] fields=this.allAttributesTrait();
           Filtre[]restrictions=new Filtre[fields.length+this.getCrit().getRestrictions().size()];
           int k=0;
           int i=0;
            for(i=0;i<fields.length;i++){
          
               restrictions[i]=new Filtre(GenCase.equal,OutilsBdd.getAttribut(fields[i]).name(),new Valeur(this.get(fields[i])));
            }  
            k=i;
        
          System.out.println(this.getCrit().getRestrictions().size()+" is the true restrictions size");
           
                 
          for(int j=0;j<this.getCrit().getRestrictions().size();j++){
                  System.out.println("Ato");
                  restrictions[k]=this.initValue(this.getCrit().getRestrictions().get(j),OutilsBdd.getField(this,this.getCrit().getRestrictions().get(j).getAttribut())) ;
                  k++;
//              if(j==this.getCrit().getRestrictions().size()-1){
//                  System.out.println("Ato ko nef");
//                  restrictions[k]=new Filtre(null,OutilsBdd.getAttribut(fields[i]).name(),new Valeur(this.get(fields[i])));
//                  k++;
//              }
          }
